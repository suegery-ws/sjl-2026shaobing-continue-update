...
static void sbus_to_rc(const uint8_t *sbus_buf)
{
    ...
    rc_ctrl[TEMP].rc.rocker_l_ = (((sbus_buf[2] >> 6) | (sbus_buf[3] << 2) | (sbus_buf[4] << 10)) & 0x07ff) - RC_CH_VALUE_OFFSET + 30; //!< Channel 2
...
RC_ctrl_t *RemoteControlInit(UART_HandleTypeDef *rc_usart_handle)
{
    ...
    Daemon_Init_Config_s daemon_conf = {
        ...
        .callback = RCLostCallback,//之后做视觉可能要对lostcallback进行改动
        .owner_id = NULL, // 只有1个遥控器,不需要owner_id
    };
    rc_daemon_instance = DaemonRegister(&daemon_conf);//将遥控器注册到检错系统中
