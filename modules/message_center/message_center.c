...
Publisher_t *PubRegister(char *name, uint8_t data_len)
{
    ...
    // 在链表尾部创建新的话题并初始化，上面这个函数已自动将指针移到最后一位，如果发现了该话题就停在那一位，结束这个函数，这就是话题的检索
    node->next_topic_node = (Publisher_t *)malloc(sizeof(Publisher_t));//创造一个新的节点
    ...
    node->pub_registered_flag = 1;
    return node->next_topic_node;//返回的是新节点的地址
    //这是创建一个链表的标准流程，memset函数是为新的链表节点申请空间，同时注册了一个新的话题发布者
}

Subscriber_t *SubRegister(char *name, uint8_t data_len)
{
    Publisher_t *pub = PubRegister(name, data_len); // 查找或创建该话题的发布者
    // 创建新的订阅者结点,申请内存,注意要memset因为新空间不一定是空的,可能有之前留存的垃圾值，使用memset一键清零
...
uint8_t SubGetMessage(Subscriber_t *sub, void *data_ptr)
{
    ...
    memcpy(data_ptr, sub->queue[sub->front_idx], sub->data_len);//选择当前想要读取的数据
    sub->front_idx = (sub->front_idx++) % QUEUE_SIZE; // 队列头索引增加 这边可能会有一个运算顺序上的bug 可以把++放前面
    sub->temp_size--;                                 // pop一个数据,长度减1，实时反馈当前消息队列长度
    return 1;
}
