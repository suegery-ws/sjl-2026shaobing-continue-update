...
typedef enum
{
    ...
    CURRENT_AND_SPEED_FEEDFORWARD = CURRENT_FEEDFORWARD | SPEED_FEEDFORWARD,//复合标志位
} Feedfoward_Type_e;
...
typedef struct
{
    Closeloop_Type_e outer_loop_type;              // 最外层的闭环,未设置时默认为最高级的闭环  111
    Closeloop_Type_e close_loop_type;              // 使用几个闭环(串级)  111
    Motor_Reverse_Flag_e motor_reverse_flag;       // 是否反转
    Feedback_Reverse_Flag_e feedback_reverse_flag; // 反馈是否反向
    Feedback_Source_e angle_feedback_source;       // 角度反馈类型  111
    Feedback_Source_e speed_feedback_source;       // 速度反馈类型  111  指针提前写
...
typedef struct
{
    float *other_angle_feedback_ptr; // 其他反馈来源的反馈数据指针(电流环)
    float *other_speed_feedback_ptr; // 其他反馈来源的反馈数据指针(速度环)//这两个在使用外部反馈时需要设置
    ...
    chassis_mode_e chassis_mode;//底盘模式的继承
...
typedef struct
{
    ...
    PID_Init_Config_s rotate_speed_PID;
