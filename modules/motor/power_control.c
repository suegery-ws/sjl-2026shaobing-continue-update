...
static void DecodeDJIMotor(CANInstance *_instance)
{
    ...
    DJIMotorInstance *motor = (DJIMotorInstance *)_instance->id;//所以说这个id就是电机实例的地址
    ...
    //解码的是大疆电机的反馈报文，并且把后续的处理函数给合并进去了
...
DJIMotorInstance *PowerControlInit(Motor_Init_Config_s *config)
{
    ...
    PIDInit(&instance->motor_controller.rotate_speed_PID, &config->controller_param_init_config.rotate_speed_PID);
    PIDInit(&instance->motor_controller.follow_speed_PID, &config->controller_param_init_config.follow_speed_PID);
    // PIDInit(&instance->motor_controller., &config->controller_param_init_config.angle_PID);//3508的初始化不需要角度环，唯一需要角度环的地方已经把角度环额外定义了,而且在主函数里面用掉了
...
void PowerControl()
{
    ...
    float pid_measure, pid_ref,pid_measure_1;             // 电机PID测量值和设定值
    ...
    {                                // 减小访存开销,先保存指针引用
        ...
        if ((motor_setting->close_loop_type & SPEED_LOOP) && (motor_setting->outer_loop_type & (ANGLE_LOOP | SPEED_LOOP)))
        {
            ...
            if (motor_setting->speed_feedback_source == OTHER_FEED)
               ...
               {
                pid_measure = measure->speed_vector; // 电机的速度单位是度每秒,转换为rpm
                pid_measure_1 = measure->speed_aps /RPM_2_RAD_PER_SEC;
               }
            // 更新pid_ref进入下一个环
            if(motor_controller->chassis_mode == CHASSIS_FOLLOW_GIMBAL_YAW || motor_controller->chassis_mode == CHASSIS_NO_FOLLOW_YAW || motor_controller->chassis_mode == CHASSIS_NO_MOVE || motor_controller->chassis_mode == CHASSIS_OPEN)
            {
            ...
            B = K2[i] * pid_measure_1 * pid_measure_1;
            C = POWER_COEF * pid_measure_1 * initial_torque[i] * TORQUE_COEF;
    ...
    if (initial_total_power > chassis_max_power)
    {
        ...
        for (uint8_t i = 0; i < idx; i++)
        {
            motor = dji_motor_instance[i];
            measure = &motor->measure;
            pid_measure_1 = measure->speed_aps /RPM_2_RAD_PER_SEC; // 电机的速度单位是度每秒,转换为rpm
            ...
            float a = K1[i];
            float b = TORQUE_COEF * POWER_COEF * pid_measure_1;
            float c = K2[i] * pid_measure_1 * pid_measure_1 - initial_give_power[i] + constant[i];
...
//     //         motor = dji_motor_instance[i];
//     //         measure = &motor->measure;
//     //         pid_measure = measure->speed_aps /6; // 电机的速度单位是度每秒,转换为rpm
//     //         initial_give_power[i] *= ratio;
//     //         if (initial_give_power[i] < 0) // 功率小于零，表明依靠发电减速，因此不算入功率统计
//     //         {
//     //             continue;
...
//                 pid_measure = *motor_controller->other_speed_feedback_ptr;
//             else // MOTOR_FEED
//                 pid_measure = measure->speed_aps / 6.0 ; // 电机的速度单位是度每秒,转换为rpm
//             // 更新pid_ref进入下一个环
...
void chassis_behavior_to_motor(Chassis_Ctrl_Cmd_s* chassis_cmd_recv)
{
    DJIMotorInstance *motor;
    ...
    {
        motor_controller->chassis_mode = CHASSIS_ZERO_FORCE; 
    }
    else if (chassis_cmd_recv->chassis_mode == CHASSIS_NO_MOVE)
    {
        motor_controller->chassis_mode = CHASSIS_NO_MOVE; 
    }
    ...
    else if (chassis_cmd_recv->chassis_mode == CHASSIS_OPEN)
    {
        motor_controller->chassis_mode = CHASSIS_OPEN;
    }
...
//      motor = dji_motor_instance[i];
//      motor_controller = &motor->motor_controller;
//      if(motor_controller->pid_ref > motor_controller->motor_limit_left)
//      {
//         motor_controller->pid_ref = motor_controller->motor_limit_left;
//      }
//      if(motor_controller->pid_ref > motor_controller->)
//      {
//         motor_controller->pid_ref = motor_controller->motor_limit_left;
//      }
//      else if (motor_controller->pid_ref < motor_controller->motor_limit_right)
//      {
//         motor_controller->pid_ref = motor_controller->motor_limit_right;
//      }
     
    

// void chassis_mode_inhert(Chassis_Ctrl_Cmd_s* chassis_cmd_recv)
// {
//     DJIMotorInstance *motor;
//     Motor_Controller_s *motor_controller;
...
//底盘3508电机模式切换//暂时不用
// static void chassis_mode_change_control_transit(Chassis_Ctrl_Cmd_s* chassis_cmd, chas)
// {
...
//     }
// 		//切入自动导航模式
// 	else if ((chassis_move_transit->last_chassis_mode != CHASSIS_AUTO_GUIDGENCE) && chassis_move_transit->chassis_mode == CHASSIS_AUTO_GUIDGENCE)
//     {
//         chassis_move_transit->chassis_yaw_set = chassis_move_transit->chassis_yaw;  
			
//     }
// 		    //切入跟随云台模式
//     else if ((chassis_move_transit->last_chassis_mode != CHASSIS_AUTO_GUIDGENCE_FOLLOW_GIMBAL_YAW) && chassis_move_transit->chassis_mode == CHASSIS_AUTO_GUIDGENCE_FOLLOW_GIMBAL_YAW)
//     {
//         chassis_move_transit->chassis_relative_angle_set = 0.0f;
//     }
