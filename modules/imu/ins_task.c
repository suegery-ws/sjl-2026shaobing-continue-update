...
attitude_t *INS_Init(void)
{
    ...
    PIDInit(&TempCtrl, &config);//温度pid初始化
...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        ...
        // VisionSetAltitude(INS.Yaw, INS.Pitch, INS.Roll); //这个是hu大的串口通讯
        TongjiVisionSetAltitude(INS.q[0], INS.q[1], INS.q[2],INS.q[3]);//四元数传输给视觉模块
