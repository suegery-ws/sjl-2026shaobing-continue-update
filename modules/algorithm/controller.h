...
typedef struct
{
    ...
    uint32_t DWT_CNT;         //计时计数器
    float dt;                 //两次PID之间的时间差，可以用来算积分和微分

    PID_ErrorHandler_t ERRORHandler;  //错误处理
} PIDInstance;
...
typedef struct // config parameter
{
    ...
    // improve parameter
    PID_Improvement_e Improve; //母济这个是干嘛的
    float IntegralLimit; // 积分限幅
...
float DMPIDCalculate(PIDInstance *pid, float measure, float ref);
float DJI2006PIDCalculate(PIDInstance *pid, float measure, float ref);
